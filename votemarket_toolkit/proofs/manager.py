...
@dataclass
class BulkProofs:
    """Proofs generated by ``VoteMarketProofs.get_proofs_bulk``.
...
class VoteMarketProofs:
    ...
    def get_proofs_bulk(
        ...
        protocol: str,
        block_number: int,
        gauge_epochs: Sequence[Tuple[str, int]] = (),
        ...
        All proofs share ``block_number``; their storage keys are grouped
        by ``keys_per_call`` into single eth_getProof calls on the gauge
        controller. The bytes are identical to ``get_gauge_proof`` /
        ``get_user_proof`` (see tests/unit/test_bulk_proofs.py).

        Args:
            protocol: The protocol name
            block_number: The block number shared by every proof
            gauge_epochs: (gauge_address, epoch) pairs for point proofs;
                epochs are rounded to the week like ``get_gauge_proof``
            users: (gauge_address, user) pairs for user proofs
            keys_per_call: Maximum storage keys per eth_getProof call
            max_retries: Attempts for a call reduced to a single proof

        Returns:
            Result[BulkProofs]: ok when everything succeeded, partial
            success when some proofs failed (see ``errors``), failure when
        ...
        if not bulk.proofs:
            return Result(success=False, data=None, errors=data.errors)
        return Result.partial_success(
            data,
            data.errors,
            total_attempted=bulk.stats.requests,
            succeeded_count=len(bulk.proofs),
        )
