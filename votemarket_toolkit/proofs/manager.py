...
class VoteMarketProofs:
    ...
    def get_gauge_proof_safe(
        ...
        """
        Generate a gauge proof with Result type for explicit error handling.

        This is the safe version of get_gauge_proof() that returns a Result
    ...
    def get_user_proof_safe(
        ...
        """
        Generate a user proof with Result type for explicit error handling.

        This is the safe version of get_user_proof() that returns a Result
        instead of raising exceptions.
    ...
    def get_block_info_safe(
        self, block_number: int, max_retries: int = 3
    ) -> Result[BlockInfo]:
        """
        Get block info with Result type for explicit error handling.
