...
async def get_all_proofs(protocol: str, campaigns: List[Dict[str, Any]], user: str) -> Dict[str, Any]:
    ...
    for campaign in campaigns:
        for claim_info in campaign["claimInfo"]:
            ...
            if epoch not in all_proofs["block_proofs"]:
                ...
                gauge_proof = vm_proofs.get_gauge_proof(protocol, gauge_address, epoch, block_number)
            ...
            if gauge_key not in all_proofs["gauge_proofs"]:
                ...
                rprint(f"[green]Generated gauge proof for {gauge_address} at epoch {epoch}[/green]")

            # Get user proof
            user_key = (epoch, gauge_address, user)
            if user_key not in all_proofs["user_proofs"]:
                user_proof = vm_proofs.get_user_proof(protocol, gauge_address, user, block_number)
                all_proofs["user_proofs"][user_key] = user_proof["storage_proof"]
                rprint(f"[green]Generated user proof for {user} on gauge {gauge_address} at epoch {epoch}[/green]")

    return all_proofs

async def build_claim_multicall(user: str, campaigns: List[Dict[str, Any]], all_proofs: Dict[str, Any]) -> List[str]:
    ...
    for campaign in campaigns:
        for claim_info in campaign["claimInfo"]:
            ...
            calldata = verifier.encodeABI(
                ...
                args=[block_proof["rlp_block_header"], block_proof["gauge_controller_proof"]],
            )
...
def encode_multicall(calldatas: List[str]) -> str:
    ...
    multicall_data = router.encodeABI(
        fn_name="multicall",
        args=[calldatas]
    )
