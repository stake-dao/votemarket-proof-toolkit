...
from votemarket_proofs.proof_generator.constants import ProofGeneratorConstants
from votemarket_proofs.proof_generator.gauge_proof import generate_gauge_proof
...
def generate_user_proof(
    ...
    vote_user_slope_base_slot = ProofGeneratorConstants.GAUGES_SLOTS[protocol][
        "vote_user_slope"
    ]
    ...
    raw_proof = w3.eth.get_proof(
        w3.to_checksum_address(
            ProofGeneratorConstants.GAUGE_CONTROLLER[protocol].lower()
        ),
        slots,
        block_number,
    )
