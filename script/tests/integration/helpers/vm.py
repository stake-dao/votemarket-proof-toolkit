...
from .web3 import W3, get_eth_balance, impersonate_account, send_transaction, send_value, set_eth_balance, stop_impersonate_account, fresh_account
...
def setup():
    ...
    send_transaction(oracle_contract.functions.setAuthorizedBlockNumberProvider(VERIFIER), GOV)
    send_transaction(oracle_contract.functions.setAuthorizedBlockNumberProvider(GOV), GOV)
...
def approve_erc20(token_address, spender, amount, from_address):
    erc20_contract = W3.eth.contract(address=token_address, abi=load_json("abi/erc20.json"))
...
def create_campaign(gauge, manager, reward_token, number_of_periods, max_reward_per_vote, total_reward_amount, addresses, hook, is_whitelist, from_address):
    votemarket_contract = W3.eth.contract(address=VOTEMARKET, abi=load_json("abi/vm_platform.json"))
    create_campaign_function = votemarket_contract.functions.createCampaign(
        42161, gauge, manager, reward_token, number_of_periods, max_reward_per_vote,
        total_reward_amount, addresses, hook, is_whitelist
    )
...
def set_block_data(rlp_block_header, controller_proof, from_address):
    verifier_contract = W3.eth.contract(address=VERIFIER, abi=load_json("abi/verifier.json"))
    set_block_data_function = verifier_contract.functions.setBlockData(rlp_block_header, controller_proof)
...
def set_point_data(gauge, epoch, storage_proof, from_address):
    verifier_contract = W3.eth.contract(address=VERIFIER, abi=load_json("abi/verifier.json"))
    set_point_data_function = verifier_contract.functions.setPointData(gauge, epoch, storage_proof)
...
def set_account_data(account, gauge, epoch, storage_proof, from_address):
    verifier_contract = W3.eth.contract(address=VERIFIER, abi=load_json("abi/verifier.json"))
    set_account_data_function = verifier_contract.functions.setAccountData(account, gauge, epoch, storage_proof)
...
def claim(campaign_id, account, epoch, hook_data, from_address):
    votemarket_contract = W3.eth.contract(address=VOTEMARKET, abi=load_json("abi/vm_platform.json"))
    claim_function = votemarket_contract.functions.claim(campaign_id, account, epoch, hook_data)
    tx_receipt = send_transaction(claim_function, from_address)
    print(f"Claim executed for campaign {campaign_id}, account {account}, and epoch {epoch}")
    return tx_receipt