...
def initialize_web3_connections(source_rpc_url=None, destination_rpc_url=None):
    ...
    if not source_rpc_url:
        ...
        if not source_rpc_url:
            raise ValueError(
                "No source RPC URL provided or found in environment"
            )
    ...
    if not destination_rpc_url:
        ...
        if not destination_rpc_url:
            raise ValueError(
                "No destination RPC URL provided or found in environment"
            )
...
def create_token_transfer(token_address: str, amount: int) -> Tuple[str, int]:
    ...
    return (to_checksum_address(token_address), amount)
...
def fetch_token_metadata(token_address: str, w3: Web3) -> Tuple[str, str, int]:
    ...
    try:
        ...
        logger.error(
            f"Error fetching metadata for token {token_address}: {str(e)}"
        )
        return ("Unknown", "???", 18)
...
def get_chain_selector(adapter_address: str, chain_id: int, w3: Web3) -> int:
    ...
    try:
        adapter_contract = w3.eth.contract(
            address=adapter_address, abi=ADAPTER_ABI
        )
...
def get_chain_id_from_selector(
    adapter_address: str, selector: int, w3: Web3
) -> int:
...
def get_next_nonce(laposte_address: str, destination_chain_id: int) -> int:
    ...
    try:
        laposte_contract = source_w3.eth.contract(
            address=laposte_address, abi=LAPOSTE_ABI
        )
        current_nonce = laposte_contract.functions.sentNonces(
...
def create_laposte_message(
    ...
    for token_address, amount in tokens:
        name, symbol, decimals = fetch_token_metadata(
            token_address, destination_w3
        )
...
def get_wrapped_token(token_address: str, token_factory_address: str) -> str:
    ...
    try:
        factory_contract = source_w3.eth.contract(
            address=token_factory_address, abi=TOKEN_FACTORY_ABI
        )
        wrapped_token = factory_contract.functions.wrappedTokens(
            token_address
        ).call()
...
def get_token_balance(token_address: str, holder_address: str) -> int:
    ...
    try:
        token_contract = source_w3.eth.contract(
            address=token_address, abi=BALANCE_ABI
        )
...
def estimate_ccip_fee(
    ...
    try:
        ...
        router_abi_path = (
            "src/votemarket_toolkit/resources/abi/ccip_router.json"
        )
        ...
        EVM_EXTRA_ARGS_V1_TAG = bytes.fromhex(
            "97a657c9"
        ...
        fee = router_contract.functions.getFee(
            destination_chain_selector, ccip_message
        ).call()
...
def simulate_ccip_receive(
    ...
    source_chain_selector = get_chain_selector(
        adapter_address, ARBITRUM_CHAIN_ID, destination_w3
    )

    # Get chain selector for destination chain (Ethereum)
    destination_chain_selector = get_chain_selector(
        adapter_address, ETHEREUM_CHAIN_ID, source_w3
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Estimate CCIP gas for token transfers"
    )
    parser.add_argument(
        "--adapter", required=True, help="Adapter contract address"
    )
    parser.add_argument(
        "--laposte", required=True, help="LaPoste contract address"
    )
    parser.add_argument(
        "--to-address", required=True, help="Recipient address"
    )
    parser.add_argument(
        "--tokens", required=True, nargs="+", help="List of token addresses"
    )
    parser.add_argument(
        "--amounts",
        required=False,
        nargs="+",
        type=int,
        help="List of token amounts (optional)",
    )
    parser.add_argument(
        "--token-factory",
